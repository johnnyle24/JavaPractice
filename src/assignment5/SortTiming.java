...
public class SortTiming {
	...
	public static void main(String[] args) {
		...
		// problem 3
		// threshold sizes analyzed: {3, 7, 11, 15, 19, 45, 80}
		// 45 and 80 were done as separate tests from the for loop below
//		for (int threshold = 3; threshold < 20; threshold += 4) {
//			// increment it by 1000 each time, 1000n, n integers from [0, 10]
...
//
//				// starts timer and sorts the worst case 100 times
//				int timesLooped = 100;
//
//				average.start();
//				for (int loop = 0; loop < timesLooped; loop++) {
//					ArrayList<Integer> temp = new ArrayList<Integer>(size);
//					temp.addAll(averageCase);
//					mergesort(temp, threshold, c);
//				}
...
//				System.out
//						.println("merge sort, average runtime of average case size = "
//								+ size
//								+ " & threshold = "
//								+ threshold
//								+ ": "
//								+ ((timeForAverage - timeForLoop) / timesLooped)
//								+ " nanoseconds.");
//			}
//		}
		// problem 4
		for (int i = 0; i <= 850; i += 50) {
			...
			System.out.println(size + "\t" + ((timeForAverage - timeForLoop) / timesLooped));
		}
	}
	 ...
	 *
	 */
	public static <T> void insertionSort(ArrayList<T> arr, int left, int right,
			Comparator<? super T> comp) {
		for (int i = left + 1; i <= right; i++) {
			T key = (T) arr.get(i);
			int j = i - 1;
			while ((j > left - 1) && (comp.compare(arr.get(j), (T) key) > 0)) {
	...
	/**
	 * This method is the driver for the mergesort. It creates the temp
	 * arraylist used in the merge and calls the recursive method.
	 * 
	 * @param arr
	 * @param comp
	 */
	...
	public static <T> void mergesort(ArrayList<T> arr,
			int insertionSortThreshold, Comparator<? super T> comp) {
		...
		mergesortRecursive(arr, 0, arr.size() - 1, insertionSortThreshold,
				comp, temp);
	}

	/**
	 * This method will call insertion sort if the size of the selected portion
	 * of the array is within a certain size. If the end is greater than the
	 * start then it will call itself twice followed by the merge method.
	 * 
	 * @param arr
	 * @param start
	 * @param end
	...
	public static <T> void mergesortRecursive(ArrayList<T> arr, int start,
		...
		if ((end - start + 1) < insertionSortThreshold) {
			insertionSort(arr, start, end, comp);
			return;
		}
		if (end > start) {
			int mid = (end + start) / 2;
			// runs the recursive call to break up into smaller arrays
			mergesortRecursive(arr, start, mid, insertionSortThreshold, comp,
					temp);
			mergesortRecursive(arr, (mid + 1), end, insertionSortThreshold,
					comp, temp);
	...
	/**
	 * This method utilizes the temporary ArrayList passed in. For the selected
	 * values, it will run comparisons based on which value is lower and sort
	 * that to left accordingly.
	 *
	 * Once done, it will copy the temporary list over to the actual ArrayList
	 * being sorted.
	 * 
	 * @param arr
	 * @param start
    ...
    public static <T> int partition(ArrayList<T> arr, int left, int right,
            ...
            //First pivot test, median value
        	//T pivot = arr.get(median(arr, left, right, comp));
            
            //Second pivot test, left side
            //T pivot = arr.get(left);
            
            //Third pivot test, random
            T pivot = arr.get(left);
	...
	/**
	 * This method generates worst case by forming an ArrayList with the
	 * integers 1 to size in reverse order.
	 *
	 * @param size
	 * @return ArrayList<Integer>
	 */
